    ...
    def next(self):
        ...
        if self.data.Close[-1] > self.data.Close[-2]:
            if len(self.resistances)==0:
                #We haven't found first resistance. Opening uptrend.
                #Wait until we get a prospective downtrend
                return
            ...
            if len(self.resistances)==0:
                ...
                if not self.position:
                    #If we don't have any position, then go short here
                    log('Go short', 'info')
                    self.sl = self.resistances[-2][1]
                    self.sell()
                else:
...
if __name__ == "__main__":
    ...
    bt = Backtest(df, ShorterBot, cash=100000, commission=0.00/100, trade_on_close=False, exclusive_orders = True)
    stats = bt.run()
    print(stats)
