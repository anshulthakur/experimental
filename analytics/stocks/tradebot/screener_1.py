...
async def main():
    ...
    # Add screener node
    screener = CustomScreen(name="Screener", filters=[RSI_Filter(value=60, greater=True), 
                                                    EMA_Filter(value=200, greater=True),
                                                    EMA_Filter(value=20, greater=True),
                                                    EMA_Filter(value=5, greater=True),])
    fg.add_node(screener)

    # Add proximity node
    proximity_up = Proximity_Screen(name="Upside-Proximity-Scanner", what='close', near='EMA5', by=0.01, direction='up', 
                                    filters=[RSI_Filter(value=60, greater=True), 
                                            EMA_Filter(value=200, greater=True),
                                            EMA_Filter(value=20, greater=True),])
    fg.add_node(proximity_up)

    # Add crossover node
    cross_up = Crossover_Screen(name="Upside-Crossover-Scanner", what='close', crosses='EMA5', direction='up', 
                                filters=[RSI_Filter(value=60, greater=True), 
                                        EMA_Filter(value=200, greater=True),
                                        EMA_Filter(value=20, greater=True),])
    ...
    screen_sink = Sink(name='Screener-Sink')
    fg.add_node(screen_sink)
    fg.register_signal_handler([EndOfData,Shutdown], screen_sink)
